...
# Didn't have the Cross Encoder and calculated the RRF score using the BGE-M3 and BM25 scores only which had an issue that either 
# the answer missed the specific company names or if remembered them then missed the context of the question due to the properties
# of the model.
...
#     def __init__(self, chroma_dir="chroma_db", bm25_path="bm25_index.pkl"):
#         print("Initializing Dense Retriever (BGE-M3 + ChromaDB)...")
#         self.embeddings = HuggingFaceEmbeddings(
#             model_name="BAAI/bge-m3",
#             encode_kwargs={'normalize_embeddings': True}
#         )
...
#     def _get_bm25_top_n(self, query, top_n=20, year_filter=None):
#         """Helper to retrieve top_n chunks from BM25 index, with optional year filtering."""
#         tokenized_query = query.lower().split()
#         scores = self.bm25.get_scores(tokenized_query)
...
#             # HARD FILTER: Skip this chunk if it doesn't match the requested year
#             if year_filter and chunk["metadata"].get("year") != year_filter:
#                 continue
...
#     def _get_chroma_top_n(self, query, top_n=20, year_filter=None):
#         """Helper to retrieve top_n chunks from Chroma, using native DB filtering."""
        
#         # Tell ChromaDB to only search documents matching this exact metadata
#         filter_dict = {"year": year_filter} if year_filter else None
        
#         chroma_res = self.vectorstore.similarity_search(
#             query, 
#             k=top_n, 
#             filter=filter_dict
#         )
...
#         results = []
#         for rank, doc in enumerate(chroma_res, 1):
#             results.append({
#                 "text": doc.page_content,
#                 "metadata": doc.metadata,
#                 "rank": rank
...
#     def rrf_search(self, query, top_k=10, k_constant=60, year_filter=None):
#         """Executes RRF fusion over Dense and Sparse results with strict metadata isolation."""
#         # Pass the filter down to both engines
#         bm25_results = self._get_bm25_top_n(query, top_n=20, year_filter=year_filter)
#         chroma_results = self._get_chroma_top_n(query, top_n=20, year_filter=year_filter)
...
#                 "bm25_rank": item["rank"],
#                 "chroma_rank": None,
#                 "rrf_score": 1.0 / (k_constant + item["rank"])
#             }

#         for item in chroma_results:
#             text = item["text"]
#             if text in fused_docs:
#                 fused_docs[text]["chroma_rank"] = item["rank"]
#                 fused_docs[text]["rrf_score"] += 1.0 / (k_constant + item["rank"])
#             else:
#                 fused_docs[text] = {
#                     "metadata": item["metadata"],
#                     "bm25_rank": None,
#                     "chroma_rank": item["rank"],
#                     "rrf_score": 1.0 / (k_constant + item["rank"])
#                 }

#         sorted_fused = sorted(fused_docs.items(), key=lambda x: x[1]["rrf_score"], reverse=True)
        
#         final_results = []
#         for text, scoring_info in sorted_fused[:top_k]:
#             final_results.append({
#                 "text": text,
#                 "metadata": scoring_info["metadata"],
#                 "rrf_score": scoring_info["rrf_score"],
#                 "bm25_rank": scoring_info["bm25_rank"],
#                 "chroma_rank": scoring_info["chroma_rank"]
#             })
...
#     # Manual test sandbox with realistic financial/operational queries
#     test_queries = [
#         "What was the organic sales growth and performance of Purina PetCare in 2024?",
#         "What are Nestlé's scope 1 and scope 2 reduction targets toward net zero?",
#         "How did Maison Perrier and Sanpellegrino perform in premium beverages during 2025?",
#         "What was the reported sales value in CHF billion for Europe in 2023?",
#         "Details on Nestlé's WHO Code compliance report and BMS marketing practices.",
#         "What product innovation came out of the internal Shark Tank initiative?",
#         "Identify supply constraints affecting Perrier water performance.",
#         "What was the UTOP margin for premium beverages in 2025?",
#         "How did pricing and RIG interact to drive growth in emerging markets during 2022?",
#         "What sales growth did the United Kingdom post in local currency in 2023?"
#     ]

#     # Run diagnostic evaluation on the first query
#     query = test_queries[0]
#     print(f"\nEvaluating Query: '{query}'\n" + "="*60)
    
#     # We explicitly tell the engine to ONLY look at 2024
#     fused_results = retriever.rrf_search(query, top_k=10, year_filter=2024)
    
#     for i, doc in enumerate(fused_results, 1):
#         meta = doc['metadata']
#         print(f"[{i}] RRF Score: {doc['rrf_score']:.5f} | BM25 Rank: {doc['bm25_rank']} | Chroma Rank: {doc['chroma_rank']}")
#         print(f"    Source: {meta['source']} | Page: {meta['page_number']} | Year: {meta['year']}")
#         print(f"    Snippet: {doc['text'][:150]}...\n" + "-"*40)
